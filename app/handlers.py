...
from app.database.requests import get_item_by_id, set_user, set_basket, get_basket, delete_basket
...
async def category(callback: CallbackQuery):
    ...
    print(await kb.items(callback.data.split('_')[1]))
...
async def mybasket(callback: CallbackQuery):
    ...
    for item_info in basket:
        ...
        counter += 1
    await callback.message.answer('Ваша корзина посту') if counter == 0 else callback.message.answer('')
